...
public class CreateBeanByDefaultConstructorProcessor extends AbstractProcessor {
    ...
    public void process(Class<?> type, BeanFactory factory) {
        ...
        if (anyAnnotationsArePresent(type)) {
            return;
        }
        putInContext(type.getConstructor().newInstance(), factory);
    }

    private boolean anyAnnotationsArePresent(Class<?> type) {
        ...
        return isAnyFieldAnnotated || autowiredPresented || isAnySetAnnotated;
    }
}
